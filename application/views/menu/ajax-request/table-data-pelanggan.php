...
<script>
    ...
    $(".update-pelanggan").click(function() {
        ...
        $.ajax({
            url: "<?= base_url(); ?>service/load_form_data_pelanggan",
            type: "get",
            success: function(data) {
    ...
    $(".add-service-pelanggan").click(function(e) {
        $("#modal-title-pelanggan").html("<i class='fas fa-tools'></i> Tambah Service Pelanggan");
        const id_pelanggan = $(this).data("idpelanggan");
        const id_mobil = $(this).data("idmobil");
        // cek apakah id_pelanggan ada dalam table spareparts service
        $.ajax({
            url: "<?= base_url() ?>service/cek_id_pelanggan",
            type: "post",
            data: {
                id_pelanggan: id_pelanggan
            },
            ...
            success: function(data) {
                if (data.status == 403) {
                    $.ajax({
                        url: "<?= base_url() ?>service/page_error_data_service_sudah_ada",
                        type: "get",
                        success: function(data) {
                            $(".modal-pelanggan").html(data);
                        }
